...
export function quantize(p: Pixels): Pixels {
  ...
  for (let i = 0; i < p.data.length; i += 4) {
    ...
    for (let c = 0; c < RGB_PALETTE.length; c++) {
      ...
      const d =
        (p.data[i] - rgb[0]) ** 2 + (p.data[i + 1] - rgb[1]) ** 2 + (p.data[i + 2] - rgb[2]) ** 2;
      if (d < distance) {
        best = c;
        distance = d;
      }
...
export async function processGeneration(
  ...
  const resized = await sharp(p.data, { raw: { width: p.width, height: p.height, channels: 4 } })
    ...
    .resize(spec.width - margin * 2, spec.height - margin * 2, {
      kernel: "nearest",
      fit: spec.transparent && spec.kind !== "backdrop" ? "contain" : "fill",
...
function frame(pose: Pixels, name: string, index: number): Pixels {
  ...
  for (let y = 0; y < h; y++)
    for (let x = 0; x < w; x++) {
      ...
      let dx = x,
        dy = Math.round(bottom - (bottom - y) * squash);
      ...
      if (dx >= 0 && dy >= 0 && dx < w && dy < h)
        pose.data.copy(p.data, (dy * w + dx) * 4, from, from + 4);
    }
...
export function packAsset(pose: Pixels, spec: AssetSpec): Pixels {
  if (spec.sheet) {
    const s = spec.sheet;
    const p = new Pixels(
      s.frameWidth * Math.max(...s.anims.map((a) => a.frames)),
      s.frameHeight * s.anims.length,
    );
          ...
          for (let y = 0; y < pose.height; y++)
            for (let x = 0; x < pose.width; x++) {
              ...
              if (pose.data[from + 3])
                pose.data.copy(
                  rendered.data,
                  (y * pose.width + Math.round(pose.width / 2 + (x - pose.width / 2) * scale)) * 4,
                  from,
                  from + 4,
                );
            }
        }
        if (spec.key === "switch" && anim.name === "on")
          rendered.rect(3, spec.height - 6, 4, 3, 23);
  ...
  if (spec.kind === "tiles") {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      if (i === 1) {
        tile.rect(0, 0, 32, 6, 4);
        tile.rect(0, 0, 32, 2, 13);
      } else if (i === 2) {
        tile.rect(0, 26, 32, 6, 3);
        for (let x = 0; x < 32; x += 8)
          for (let y = 0; y < 12; y++)
            tile.rect(x + 4 - Math.floor(y / 3), 14 + y, 1 + Math.floor(y / 3) * 2, 1, 30);
      } else if (i === 3) {
        tile.rect(0, 0, 32, 32, 1);
        for (let x = 3; x < 32; x += 8) {
          tile.rect(x, 0, 4, 32, 4);
          tile.rect(x, 0, 1, 32, 18);
        }
      } else {
        tile.blit(pose, 0, 0);
  ...
  if (spec.kind === "billboards") {
    ...
    BILLBOARD_SLOGANS.forEach((slogan, i) => {
      ...
      const lines =
        slogan === "PAUSE EVERYTHING"
          ? ["PAUSE", "EVERYTHING"]
          : slogan === "NO DATACENTERS"
            ? ["NO", "DATACENTERS"]
            : [slogan];
      lines.forEach((line, j) =>
        text(
          panel,
          line,
          Math.floor((panel.width - (line.length * 6 - 1) * 2) / 2),
          lines.length === 1 ? 31 : 20 + j * 22,
          i === 0 ? 18 : i === 1 ? 27 : 23,
          2,
        ),
      );
