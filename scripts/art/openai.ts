...
export async function availableModels(key: string, request: typeof fetch = fetch): Promise<ImageModel[]> {
...
export async function generateImage(
  spec: AssetSpec, model: ImageModel, key: string, cacheDir: string,
  revision: string, request: typeof fetch = fetch,
): Promise<Generation> {
  const candidates = (spec.transparent ? ["transparent", "opaque"] as const : ["opaque"] as const).map((background) => {
    ...
    const body = { model, prompt, n: 1, size: "1024x1024", quality: "medium", output_format: "png", background };
  ...
  for (const { filename, metadata } of candidates) {
    try { return { ...metadata, raw: await readFile(filename) }; }
