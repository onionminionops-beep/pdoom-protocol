...
function renderActions(player: PlayerState, slot: SlotKind = "JEV") {
  ...
  element.innerHTML = renderToStaticMarkup(createElement(JevActions, { player, slot }));
  return element;
}
...
describe("JEV action HUD", () => {
  it("shows simultaneous applied inputs and clears them on the next neutral snapshot", () => {
    ...
    expect(actionValues(active)).toEqual(["← Left", "Jump held", "On", "On", "On"]);
    ...
    expect(actionValues(renderActions(player))).toEqual(["Right →", "Drop", "On", "On", "On"]);
  ...
  it.each(["Disabled", "Downed", "Dead"] as const)("masks retained inputs when %s", (state) => {
    const player = createPlayer("p2", 0, 0);
    player.lastInput = {
      ...neutralInput("episode", 1),
      horizontal: "right",
    ...
    const element = renderActions(player, state === "Disabled" ? "DISABLED" : "JEV");
  ...
  it("distinguishes waiting for the first input from idle and avoids live announcements", () => {
    ...
    expect(element.querySelector("section")?.getAttribute("aria-live")).toBe("off");
    expect(element.querySelector('[role="status"],[role="alert"],button,kbd')).toBeNull();
  });
  ...
  it("uses only p2's applied input even when model/debug data disagrees", () => {
    ...
    expect(element.querySelectorAll('[aria-label="JEV applied actions"]')).toHaveLength(1);
    const panel = element.querySelector('[aria-label="JEV applied actions"]')!;
    expect(panel.closest(".p2")).not.toBeNull();
    expect(panel.previousElementSibling?.getAttribute("aria-label")).toBe("JEV health");
    expect(actionValues(panel)).toEqual(["← Left", "None", "Off", "Off", "Off"]);
  });
});
...
describe("JEV mode selector", () => {
  it("exposes all five modes using stable directive values and an accessible label", () => {
    ...
    expect([...select.options].map((option) => option.value)).toEqual(Object.keys(DIRECTIVES));
