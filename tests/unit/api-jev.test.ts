...
vi.mock("@/server/jev/limits", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@/server/jev/limits")>()),
  getJevLimits: mocks.getJevLimits,
}));
...
describe("signed Jev sessions", () => {
  it.each([
    ...
    "advertises pacing for $budget requests and $perMinute/minute without changing signed budgets",
    async ({ budget, perMinute, interval }) => {
  ...
  it.each<Record<string, string>>([
    ...
    await expectError(
      await sessionPost(request({ episodeId: "ep-test" }, headers)),
      403,
      "forbidden_origin",
    );
  });
...
describe("decision route", () => {
  it.each([
    ...
    {
      name: "Vercel with development NODE_ENV",
      nodeEnv: "development",
      vercel: "1",
      vercelEnv: "",
    },
    ...
    "fails closed for both routes without Redis in $name",
    async ({ nodeEnv, vercel, vercelEnv }) => {
  ...
  it.each(["UPSTASH_REDIS_REST_URL", "UPSTASH_REDIS_REST_TOKEN"])(
    "rejects partial Redis configuration with only %s",
    async (name) => {
  ...
  it.each([
    ...
    { schemaVersion: "2.0" },
    { self: { character: "user" } },
    { directive: { id: "EVIL", description: "" } },
    { hostileProjectiles: Array(9).fill({}) },
  ])("rejects malformed observations before inference %j", async (patch) => {
    const { sessionToken } = await session();
    await expectError(
      await decisionPost(request({ sessionToken, observation: { ...observation(), ...patch } })),
      400,
      "invalid_request",
    );
    expect(mocks.requestDecision).not.toHaveBeenCalled();
  ...
  it("rejects malformed JSON and unsupported content types", async () => {
    ...
    await expectError(
      await decisionPost(request({}, { "content-type": "text/plain" })),
      400,
      "invalid_request",
    );
  });
  ...
  it("rejects unknown sessions and mismatched episodes without calling TypeSafe", async () => {
    ...
    await expectError(
      await decisionPost(request({ sessionToken: unknown.token, observation: observation() })),
      401,
      "invalid_session",
    );
    const { sessionToken } = await session();
    await expectError(
      await decisionPost(
        request({
          sessionToken,
          observation: { ...observation(), episodeId: "old-episode" },
        }),
      ),
      401,
      "invalid_session",
  ...
  it("allows only one concurrent claim of a tick and enforces the session budget", async () => {
    ...
    await expectError(
      await decisionPost(request({ sessionToken, observation: { ...obs, tick: 2 } })),
      429,
  ...
  it("sanitizes unexpected and upstream timeout errors", async () => {
    ...
    mocks.requestDecision.mockRejectedValueOnce(
      new JevApiError("upstream_timeout", 504, "Jev took too long to respond."),
    );
    await expectError(
      await decisionPost(
        request({
          sessionToken,
          observation: { ...observation(), tick: 1 },
        }),
      ),
      504,
      "upstream_timeout",
