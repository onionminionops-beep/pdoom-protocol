...
function mk(slots: Parameters<typeof createWorld>[0]["slots"] = { p1: "HUMAN", p2: "MOCK_AI" }) {
  return createWorld({ level: CONSENSUS_HEIGHTS, seed: 42, episodeId: "ep-test", directive: "SPEEDRUNNER", slots });
}
...
describe("simulation", () => {
  it("is deterministic for identical inputs", () => {
    ...
    const b = mk();
    for (let t = 0; t < 600; t++) {
      const i1 = input("ep-test", t, { horizontal: "right", shoot: t % 20 === 0, verticalAction: t % 90 === 0 ? "jump" : "none" });
  ...
  it("uses identical physics for User and JEV", () => {
    ...
    for (let t = 0; t < 120; t++) {
      const i = input("ep-test", t, { horizontal: "right", verticalAction: t < 5 ? "jump" : "none" });
  ...
  it("only changes facing on explicit horizontal input and fires along facing", () => {
    ...
    stepWorld(w, { p1: input("ep-test", 0, { horizontal: "left" }), p2: neutralInput("ep-test", 0) });
    expect(p.facing).toBe("left");
    for (let t = 1; t < 30; t++) stepWorld(w, { p1: input("ep-test", t, { horizontal: "neutral" }), p2: neutralInput("ep-test", t) });
    expect(p.facing).toBe("left");
    const ev = stepWorld(w, { p1: input("ep-test", 30, { shoot: true }), p2: neutralInput("ep-test", 30) });
  ...
  it("ignores inputs from a stale episode", () => {
    ...
    for (let t = 0; t < 30; t++) stepWorld(w, { p1: input("old-episode", t, { horizontal: "right" }), p2: neutralInput("ep-test", t) });
  ...
  it("does not let friendly launcher fire hurt the teammate", () => {
    ...
    for (let t = 0; t < 120; t++) stepWorld(w, { p1: input("ep-test", t, { shoot: true }), p2: neutralInput("ep-test", t) });
    expect(w.players.p2.health).toBe(hp);
  });

  it("produces a valid, deterministic observation with relative coordinates", () => {
    const w = mk();
    for (let t = 0; t < 60; t++) stepWorld(w, { p1: input("ep-test", t, { horizontal: "right" }), p2: neutralInput("ep-test", t) });
    ...
    for (let i = 1; i < o1.pickups.length; i++) expect(o1.pickups[i].distance).toBeGreaterThanOrEqual(o1.pickups[i - 1].distance);
  });
  ...
  it("mock AI plays legally through the input contract and makes progress", () => {
    ...
    for (let t = 0; t < 60 * 20; t++) {
      const inp = mock.update({ world: w, playerId: "p2", tick: w.tick, episodeId: w.episodeId, nowMs: t * (1000 / 60) });
  ...
  it("accepts older observations that omit held jump state", () => {
    ...
    const legacySelf = Object.fromEntries(Object.entries(obs.self).filter(([key]) => key !== "jumpHeld"));
  ...
  it("holds mock jump through ascent and clears the tutorial ledge like a human", () => {
    ...
    for (let t = 0; t < 45; t++) {
      ...
      stepWorld(w, {
        p1: input(w.episodeId, w.tick, { horizontal: "right", verticalAction: t < 20 ? "jump" : "none" }),
        p2: inp,
      });
