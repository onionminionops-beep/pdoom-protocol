...
describe("live directive changes", () => {
  ...
  it("rejects late prior-mode decisions even if transport ignores abort", async () => {
    ...
    const nextRequest = DecisionRequestSchema.parse(
      JSON.parse(String(requests[3].init?.body)),
    );
    ...
    session.publish();
    expect(snapshots.mock.lastCall?.[0].jevStatus.lastRequestId).toBe(
      "request-3",
    );
  });
  ...
  it("discards already held AI input and ignores superseded session connections on rapid switches", async () => {
    ...
    session.advance(TICK_MS, performance.now());
    expect(
      requests.filter((request) => request.url === "/api/jev/decision"),
    ).toHaveLength(2);
    const { observation } = DecisionRequestSchema.parse(
      JSON.parse(String(requests[4].init?.body)),
    );
...
describe("directive contract", () => {
  it.each(DirectiveIdSchema.options)(
    ...
    (directive) => {
      ...
      const observation = GameObservationV1Schema.parse(
        buildObservation(session.world, "p2", 0),
      );
