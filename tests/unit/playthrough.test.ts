...
describe("Consensus Heights playthrough", () => {
  it.each(["scripted shared inputs", "two observation-only mock teammates"])(
    "wins with %s",
    (mode) => {
      const run = runPlaythrough(
        mode === "scripted shared inputs" ? { driver: scriptedDriver() } : {},
      );
      ...
      expect(run.world.openedGates).toEqual({
        r2_comments: true,
        r4_gallery: true,
        r6_split: true,
      });
      expect(run.events).toContainEqual(
        expect.objectContaining({ type: "interact", playerId: "p1", interactableId: "sw_upper" }),
      );
      expect(run.events).toContainEqual(
        expect.objectContaining({ type: "interact", playerId: "p2", interactableId: "sw_lower" }),
      );
      expect(run.events).toContainEqual(
        expect.objectContaining({ type: "enemy_killed", enemyType: "consensus_engine" }),
      );
      expect(run.world.bossActive).toBe(false);
      expect(run.world.enemies.some((e) => e.type === "consensus_engine" && e.health > 0)).toBe(
        false,
      );
      ...
      expect(run.world.score).toBe(
        b.kills + b.coins + b.timeBonus + b.reviveBonus - b.damageTakenPenalty,
      );
    },
    30_000,
  );
  ...
  it("uses shared jump constants that clear the largest required three-tile rise", () => {
    ...
    expect(MOVEMENT.runSpeed * ((-2 * MOVEMENT.jumpVelocity) / MOVEMENT.gravity)).toBeGreaterThan(
      128,
    );
  });
});
