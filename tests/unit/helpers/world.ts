...
export function flatLevel(): LevelData {
  return {
    ...
    rooms: [{
      id: "arena", bounds: [0, 0, 96, 18], objectiveType: "traverse",
      objectiveText: "Traverse", gateOnEnemies: false, gateTileX: null, billboards: [],
    }],
    enemies: [], pickups: [], interactables: [], exitTileX: 95,
  };
}

export function arena(options: Partial<CreateWorldOptions> = {}): WorldState {
  return createWorld({
    level: flatLevel(), seed: 42, episodeId: "arena", directive: "SPEEDRUNNER",
    slots: { p1: "HUMAN", p2: "MOCK_AI" }, ...options,
  });
}

export function advance(world: WorldState, ticks = 1, p1: Partial<PlayerInputV1> = {}, p2: Partial<PlayerInputV1> = {}) {
  return Array.from({ length: ticks }, () => stepWorld(world, {
