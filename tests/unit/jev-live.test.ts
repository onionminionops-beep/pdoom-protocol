...
it.runIf(process.env.JEV_LIVE_TEST === "1")("verifies one real TypeSafe decision", async () => {
  if (!process.env.TYPESAFE_API_KEY) throw new Error("Live verification requires a server API key.");
  const obs = buildObservation(createWorld({
    level: CONSENSUS_HEIGHTS, seed: 42, episodeId: "live-verification", directive: "SPEEDRUNNER",
  ...
  const client = new TypeSafeClient({
    ...
    fetch: async (url, init) => {
      ...
      if (response.ok) process.stdout.write(`TypeSafe response: ${JSON.stringify(await response.clone().json())}\n`);
