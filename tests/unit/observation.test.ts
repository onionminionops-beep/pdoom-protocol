...
import { GameObservationV1Schema, OBSERVATION_LIMITS, serializeObservation } from "@/game/contracts/observation";
...
describe("bounded observation contract", () => {
  it("orders enemy threats by time to contact then distance before truncating", () => {
    ...
    for (let i = 12; i > 0; i--) {
      const e = spawnEnemy(world, `enemy-${i}`, "doom_prophet", p.pos.x + 21 + i * 20, p.pos.y, "arena");
      e.vel.x = i <= 3 ? -i * 40 : 0;
    }
    const obs = buildObservation(world, "p2", 0);
    expect(obs.enemies).toHaveLength(OBSERVATION_LIMITS.enemies);
    expect(obs.enemies.map((e) => e.id)).toEqual(Array.from({ length: 8 }, (_, i) => `enemy-${i + 1}`));
  ...
  it("orders approaching projectiles first, breaking equal or unknown times by distance", () => {
    ...
    for (let i = 12; i > 0; i--) {
      ...
      fireEnemyProjectile(world, source, "enemy_graph", { x: i <= 3 ? -i * 40 : 100, y: 0 }, 12, 6, 2000);
    ...
    expect(obs.hostileProjectiles.map((pr) => pr.id)).toEqual(Array.from({ length: 8 }, (_, i) => `projectile-${i + 1}`));
  ...
  it("limits pickups/interactables to visible, nearest, permitted objects", () => {
    ...
    const p = world.players.p2;
    for (let i = 12; i > 0; i--) {
      world.pickups.push({ id: `pickup-${i}`, type: "coin", value: 1, pos: { x: p.pos.x + 20 * i, y: p.pos.y }, collected: false, roomId: "arena" });
      world.interactables.push({
        id: `switch-${i}`, type: "switch", pos: { x: p.pos.x + 20 * i, y: p.pos.y },
        w: 32, h: 32, activated: false, requiredPlayer: i === 1 ? "p1" : "p2", label: "Switch", roomId: "arena",
      });
    }
    world.pickups.push({ ...world.pickups[0], id: "offscreen", pos: { x: p.pos.x + 500, y: p.pos.y } });
    const obs = buildObservation(world, "p2", 123);
    expect(obs.pickups.map((it) => it.id)).toEqual(["pickup-1", "pickup-2", "pickup-3"]);
    expect(obs.interactables.map((it) => it.id)).toEqual(["switch-2", "switch-3", "switch-4", "switch-5"]);
