...
export default function GameShell() {
  ...
  const completedEpisode = useRef<string | null>(null);
  ...
  function launch(runOptions: SessionOptions) {
    ...
    const next = new ClientSession(runOptions, settings, engine, developer);
    next.onSnapshot = (value) => {
      setSnapshot(value);
      if (value.comparison && completedEpisode.current !== value.world.episodeId) {
        completedEpisode.current = value.world.episodeId;
        const result = value.comparison;
        setComparisons((previous) => [...previous, result]);
      }
    };
  ...
  function updateSettings(value: ClientSettings) {
    ...
    audio.current?.setMasterVolume(value.master);
    audio.current?.setMusicVolume(value.music);
    audio.current?.setSfxVolume(value.sfx);
  }
  ...
  function compare(directive: DirectiveId) {
    const companion = options.slots.p2;
    if (!replay || (companion !== "JEV" && companion !== "MOCK_AI")) return;
