    ...
    >
      ...
      <div className="volume-controls">
        {(["master", "music", "sfx"] as const).map((channel) => (
          <label key={channel}>
            {channel.toUpperCase()} <span>{Math.round(settings[channel] * 100)}%</span>
            <input
              ...
              onChange={(event) => onChange({ ...settings, [channel]: Number(event.target.value) })}
            />
          </label>
        ))}
      </div>
      <div className="visual-settings">
        <label>
          <input
            ...
            onChange={(event) => onChange({ ...settings, screenShake: event.target.checked })}
          />{" "}
          Screen shake
        </label>
        <label>
          <input
            ...
            onChange={(event) => onChange({ ...settings, reducedFlashing: event.target.checked })}
          />{" "}
          Reduced flashing
        </label>
        <label>
          <input
            ...
            onChange={(event) => onChange({ ...settings, colorblind: event.target.checked })}
          />{" "}
      ...
      <p role="status" className="settings-message">
        {message ||
          (remapping
            ...
            : "Changes apply immediately for this visit.")}
      </p>
      <div className="dialog-actions">
        ...
        <button
          ...
          onClick={() =>
            onChange(defaultSettings(window.matchMedia("(prefers-reduced-motion: reduce)").matches))
          }
        >
          Reset defaults
        </button>
