...
import { detectLanding, otherPlayerId, playerBox, stepPlayer, tryFactCheckPulse, tryShoot } from "./player";
...
export const REVIVE_HOLD_MS = 1500;
export const REVIVE_RANGE_PX = 40;
export const INTERACT_RANGE_PX = 36;
...
function sanitize(world: WorldState, pid: PlayerId, input: PlayerInputV1 | undefined): PlayerInputV1 {
...
function clampToLevel(world: WorldState, p: { pos: { x: number; y: number }; vel: { x: number; y: number } }): void {
...
function handleRevives(world: WorldState, inputs: PlayerInputs, events: SimEvent[]): void {
  for (const pid of ["p1", "p2"] as const) {
    ...
    const canRevive =
      ...
      other.alive &&
      other.downed &&
      Math.hypot(other.pos.x - p.pos.x, other.pos.y - p.pos.y) < REVIVE_RANGE_PX + MOVEMENT.bodyWidth &&
      inputs[pid].interact;
    if (canRevive) {
      p.reviveProgressMs += TICK_MS;
      other.anim = "being_revived";
      events.push({ type: "revive_progress", playerId: pid, target: oid, fraction: Math.min(1, p.reviveProgressMs / REVIVE_HOLD_MS) });
      if (p.reviveProgressMs >= REVIVE_HOLD_MS) {
        other.downed = false;
        other.health = Math.round(MOVEMENT.maxHealth * 0.5);
...
function handleInteractables(world: WorldState, inputs: PlayerInputs, events: SimEvent[]): void {
  for (const it of world.interactables) {
    ...
    for (const pid of ["p1", "p2"] as const) {
      ...
      const box = { x: it.pos.x, y: it.pos.y, w: it.w + INTERACT_RANGE_PX, h: it.h + INTERACT_RANGE_PX };
      if (!aabbOverlap(box, playerBox(p))) continue;
      if (it.type === "exit" && world.enemies.some((e) => e.type === "consensus_engine" && e.health > 0)) continue;
      it.activated = true;
      events.push({ type: "interact", playerId: pid, interactableId: it.id, interactableType: it.type });
      if (it.type === "weapon_crate") {
        ...
        const roomSwitches = world.interactables.filter((s) => s.type === "switch" && s.roomId === it.roomId);
        if (room && room.gateTileX !== null && !room.gateOnEnemies && roomSwitches.every((s) => s.activated) && !world.openedGates[room.id]) {
    ...
    for (const pid of ["p1", "p2"] as const) {
      ...
      if (pk.type === "coin") {
        ...
        for (const w of ["shotgun", "launcher"] as const) {
          if (p.ammo[w] !== null) p.ammo[w] = Math.min(WEAPONS[w].maxAmmo ?? 0, (p.ammo[w] ?? 0) + WEAPONS[w].ammoPerPickup);
        }
      ...
      events.push({ type: "pickup", playerId: pid, pickupId: pk.id, pickupType: pk.type, value: pk.value });
      break;
    }
  }
}
...
function stepZonesAndHazards(world: WorldState, events: SimEvent[]): void {
  ...
  for (const pid of ["p1", "p2"] as const) {
    ...
    const hazard = row !== undefined && row[tx] === "H";
    if (hazard || p.pos.y > world.level.heightTiles * TILE + 64) {
      p.health = 0;
      p.downed = true;
      p.downedTimerMs = 20000;
      p.damageTaken += 100;
      // Pull the body back onto the last safe ground so a teammate can revive.
      p.pos.y = Math.min(p.pos.y, (ty - 1) * TILE + TILE - MOVEMENT.bodyHeight / 2);
      p.vel = { x: 0, y: 0 };
      events.push({ type: "player_hurt", playerId: pid, damage: 100, from: "hazard" });
      events.push({ type: "player_downed", playerId: pid });
      continue;
    }
...
function updateRooms(world: WorldState, events: SimEvent[]): void {
  ...
  for (const room of world.level.rooms) {
    ...
    if (room.gateOnEnemies && !world.openedGates[room.id]) {
      ...
      if (!remaining && world.enemies.length >= 0 && world.tick > 1) {
        // Only open once the room has been entered (avoid opening before spawn logic).
        if (world.currentRoomId === room.id || roomIndex(world, room.id) < roomIndex(world, world.currentRoomId)) {
          world.openedGates[room.id] = true;
          openGateColumn(world, room.gateTileX);
          events.push({ type: "gate_opened", roomId: room.id });
        }
...
export function isGateClosedAt(world: WorldState, tileX: number): boolean {
  for (const room of world.level.rooms) {
    if (room.gateTileX === tileX && room.gateOnEnemies && !world.openedGates[room.id]) return true;
  }
  return false;
}
...
function finalizeScore(world: WorldState): void {
  ...
  world.score = world.breakdown.kills + world.breakdown.coins + world.breakdown.reviveBonus + timeBonus - penalty;
}
...
function checkEndConditions(world: WorldState, events: SimEvent[]): void {
  ...
  const anyDownedAlive = active.some((pid) => world.players[pid].alive && world.players[pid].downed);
