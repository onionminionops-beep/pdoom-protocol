...
export const renderManifestSchema = z.object({
  ...
  billboards: z.object({
    file: assetPath, frameWidth: positive, frameHeight: positive, count: positive,
  }).optional(),
  ui: z.object({
    font: z.string(), portraitUser: assetPath, portraitJev: assetPath, logo: assetPath,
  }).optional(),
});
...
export function spritePlacement(sheet: SpriteSheetDef, x: number, y: number, flip: boolean, bodyHeight?: number) {
  return {
...
export function pixelDisplayScale(width: number, height: number, availableWidth: number, availableHeight: number): number {
