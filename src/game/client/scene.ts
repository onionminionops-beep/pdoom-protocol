...
import { animationFrame, parseRenderManifest, pixelDisplayScale, sheetEntries, spritePlacement, type RenderManifest } from "./art";
...
export class ProtocolScene extends Phaser.Scene {
  ...
  update(_time: number, delta: number): void {
    ...
    for (const item of world.interactables) {
      ...
      if (!this.sprite(item.type, item.pos.x, item.pos.y, anim, world.elapsedMs - state.since, false, 0xffffff, 1, item.h)) {
    ...
    for (const enemy of world.enemies) {
      ...
      if (!this.sprite(enemy.type, enemy.pos.x, enemy.pos.y, hurt ? "hurt" : enemy.phase, hurt ? 180 - enemy.hurtMs : enemy.phaseMs, enemy.facing === "left", tint, alpha, enemy.h, hurt)) {
      ...
      if (enemy.health > 0) {
        this.overlay.fillStyle(0x382050).fillRect(Math.round(enemy.pos.x - 15), Math.round(enemy.pos.y - enemy.h / 2 - 7), 30, 3);
        this.overlay.fillStyle(color).fillRect(Math.round(enemy.pos.x - 15), Math.round(enemy.pos.y - enemy.h / 2 - 7), Math.round(30 * enemy.health / enemy.maxHealth), 3);
      }
      ...
      if (this.session.hitboxes) this.overlay.lineStyle(1, 0xef6262).strokeRect(enemy.pos.x - enemy.w / 2, enemy.pos.y - enemy.h / 2, enemy.w, enemy.h);
    }
    ...
    for (const projectile of world.projectiles) {
      ...
      if (!this.sprite("projectiles", projectile.pos.x, projectile.pos.y, projectile.weapon, projectile.ageMs, projectile.vel.x < 0, 0xffffff)) {
      ...
      this.overlay.lineStyle(1, color, 0.8).strokeCircle(projectile.pos.x, projectile.pos.y, projectile.radius);
      if (this.session.hitboxes) this.overlay.lineStyle(1, 0xffffff).strokeCircle(projectile.pos.x, projectile.pos.y, projectile.radius);
    }
    for (const effect of this.effects) {
      ...
      if (!this.session.settings.reducedFlashing) this.sprite("fx", effect.x, effect.y, effect.anim, elapsed, effect.flip, 0xffffff, alpha);
      this.overlay.lineStyle(2, 0xf3b65d, alpha * 0.7).strokeCircle(effect.x, effect.y, effect.radius * elapsed / effect.duration);
    }
    this.effects = this.effects.filter((effect) => world.elapsedMs - effect.startedAt < effect.duration);
  ...
  private label(x: number, y: number, text: string, color: string, size: number, bubble = false): void {
    ...
    label.setVisible(true).setPosition(Math.round(x), Math.round(y)).setText(text).setStyle({
      fontFamily: this.manifest.ui?.font ?? "monospace", fontSize: `${size}px`, color,
  ...
  private fitCanvas(): void {
    ...
    if (!parent) return;
    const scale = pixelDisplayScale(this.scale.width, this.scale.height, parent.clientWidth, parent.clientHeight);
    canvas.style.width = `${this.scale.width * scale}px`;
    canvas.style.height = `${this.scale.height * scale}px`;
  }

  private sprite(key: string, x: number, y: number, anim: string, elapsed: number, flip = false, tint = 0xffffff, alpha = 1, bodyHeight?: number, flash = false): boolean {
    ...
    const frame = animationFrame(definition, anim, this.session.settings.reducedFlashing ? 0 : elapsed, columns);
    const placement = spritePlacement(definition, x, y, flip, bodyHeight);
    const image = this.image(`art:${key}`, placement.x, placement.y, texture.has(String(frame)) ? frame : 0, key === "fx" ? 3 : 1)
      .setOrigin(0.5, placement.originY).setFlipX(flip).setTint(tint).setAlpha(alpha);
  ...
  private drawTile(tile: string, x: number, y: number): void {
    ...
    const kind = tile === "#" ? (rows[y - 1]?.[x] !== "#" ? "solid_top" : rows[y]?.[x - 1] !== "#" ? "solid_left" : rows[y]?.[x + 1] !== "#" ? "solid_right" : "solid_inner") : tile === "-" ? "oneway" : tile === "H" ? "hazard" : "gate";
  ...
  private drawPlayer(player: PlayerState): void {
    ...
    const anim = !player.alive ? "death" : this.session.world.status === "won" ? "victory" : player.anim;
    if (!this.sprite(player.character, x, y, anim, player.animMs, facing < 0, 0xffffff, player.alive ? 1 : 0.4, MOVEMENT.bodyHeight, hurt)) {
    ...
    if (player.downed) {
      ...
      this.overlay.lineStyle(4, color).beginPath().arc(x, y, 30, -Math.PI / 2, -Math.PI / 2 + Math.PI * 2 * Math.min(1, other.reviveProgressMs / REVIVE_HOLD_MS), false).strokePath();
    }
    if (this.session.hitboxes) this.overlay.lineStyle(1, 0xffffff).strokeRect(x - MOVEMENT.bodyWidth / 2, y - MOVEMENT.bodyHeight / 2, MOVEMENT.bodyWidth, MOVEMENT.bodyHeight);
  }
    ...
    for (let layer = 0; layer < 3; layer++) {
      ...
      if (definition && this.textures.exists(`art:${artKey}`)) {
        ...
        for (let x = -artOffset; x < width; x += tileWidth) {
          this.image(`art:${artKey}`, x, height - definition.height * scale, 0, -8 + layer).setOrigin(0).setScrollFactor(0).setScale(scale);
        }
        continue;
      }
  ...
  private consumeEvents(events: SimEvent[]): void {
    ...
    for (const event of events) {
      if (event.type === "explosion" || event.type === "projectile_hit") {
        this.effects.push({ ...event.pos, radius: event.type === "explosion" ? event.radius : 14, anim: event.type === "explosion" ? "explosion" : "impact", duration: 500, startedAt });
      } else if (event.type === "fact_check_pulse") {
        this.effects.push({ ...event.pos, radius: FACT_CHECK_PULSE.radius, anim: "pulse", duration: 500, startedAt });
      } else if (event.type === "dash") {
        const player = world.players[event.playerId];
        this.effects.push({ x: player.pos.x + (event.facing === "left" ? 16 : -16), y: player.pos.y, radius: 20, anim: "dash", flip: event.facing === "left", duration: 500, startedAt });
      }
