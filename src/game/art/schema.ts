...
export const SpriteSheetSchema = z.object({
  ...
  if (sheet.anims.some((anim, index) => anim.row !== index)) {
    ctx.addIssue({ code: "custom", message: "Animation rows must be contiguous and ordered", path: ["anims"] });
  }
...
export const ArtManifestSchema = z.object({
  ...
  props: z.record(z.enum(["coin", "health", "ammo", "fact_check", "weapon_crate", "switch", "exit", "projectiles", "fx"]), SpriteSheetSchema),
  tileset: z.object({
    key: z.literal("tiles"), file, tileSize: z.literal(32), columns: positiveInt,
    tiles: z.record(z.enum(["solid", "oneway", "hazard", "gate", "solid_top", "solid_left", "solid_right", "solid_inner"]), z.array(z.number().int().nonnegative()).min(1)),
  }),
  backdrops: z.object({ bg_far: backdrop, bg_mid: backdrop, bg_near: backdrop }),
  billboards: z.object({ file, frameWidth: positiveInt, frameHeight: positiveInt, count: positiveInt }),
  ui: z.object({ font: z.string().min(1), portraitUser: file, portraitJev: file, logo: file }),
}).superRefine((manifest, ctx) => {
  for (const [key, sheet] of Object.entries({ ...manifest.characters, ...manifest.enemies, ...manifest.props })) {
    if (sheet.key !== key) ctx.addIssue({ code: "custom", message: `Sheet key must be ${key}` });
    const expected = key in manifest.characters ? PLAYER_ANIMS : key in manifest.enemies ? ENEMY_ANIMS : null;
