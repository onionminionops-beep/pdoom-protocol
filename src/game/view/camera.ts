...
export function fitCamera(world: WorldState): CameraBounds {
  const active = Object.values(world.players).filter(
    (p) => world.slots[p.id] !== "DISABLED" && p.alive,
  );
  ...
  const factor = Math.max(
    1,
    Math.ceil((maxX - minX + 160) / VIEW_WIDTH),
    Math.ceil((maxY - minY + 160) / VIEW_HEIGHT),
  );
  ...
  const clamp = (center: number, extent: number, level: number) =>
    extent > level
      ? (level - extent) / 2
      : Math.max(0, Math.min(level - extent, center - extent / 2));
  return {
    width,
    height,
    factor,
...
export function cameraContains(camera: CameraBounds, pos: { x: number; y: number }): boolean {
  return (
    pos.x >= camera.x &&
